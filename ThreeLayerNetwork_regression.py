# -*- coding: utf-8 -*-
"""
Created on Sat Apr  4 19:53:52 2020
...
class ThreeLayerNet(object):
  ...
  def __init__(self, input_size, hidden1_size, hidden2_size, output_size, std=1e-1):
      '''
      ...
      '''
      self.params = {}
      self.params['W1'] = np.random.randn(input_size, hidden1_size) / np.sqrt(input_size)#-1, 1, size = [input_size, hidden1_size])
      self.params['b1'] = np.zeros(hidden1_size)
      self.params['W2'] = np.random.randn(hidden1_size, hidden2_size) / np.sqrt(hidden1_size)#-1, 1, size = [hidden1_size, hidden2_size])
      self.params['b2'] = np.zeros(hidden2_size)
      self.params['W3'] = np.random.randn(hidden2_size, output_size) / np.sqrt(hidden2_size)#-1, 1, size = [hidden2_size, output_size])
  ...
  def loss(self, X, y, reg=0.0):
    ...
    - grads: Dictionary mapping parameter names to gradients of those parameters
      ...
      if y.dtype=='int':
          ...
          data_loss = np.sum((scores-y[:,np.newaxis])*(scores-y[:,np.newaxis])) / N
